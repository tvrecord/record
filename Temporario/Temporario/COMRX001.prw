...
, CASE WHEN D1_TIPO <> 'D' THEN (SELECT A2_NOME FROM %table:SA2% WHERE D1_FORNECE = A2_COD AND D1_LOJA = A2_LOJA AND %table:SA2%.D_E_L_E_T_ = '' )
WHEN D1_TIPO = 'D' THEN (SELECT A1_NOME FROM %table:SA1% WHERE D1_FORNECE = A1_COD AND D1_LOJA = A1_LOJA AND %table:SA1%.D_E_L_E_T_ = '' ) END AS NOME
, CASE WHEN D1_TIPO <> 'D' THEN (SELECT A2_EST FROM %table:SA2% WHERE D1_FORNECE = A2_COD AND D1_LOJA = A2_LOJA AND %table:SA2%.D_E_L_E_T_ = '' )
WHEN D1_TIPO = 'D' THEN (SELECT A1_EST FROM %table:SA1% WHERE D1_FORNECE = A1_COD AND D1_LOJA = A1_LOJA AND %table:SA1%.D_E_L_E_T_ = '' ) END AS ESTADO
, CASE WHEN D1_TIPO <> 'D' THEN (SELECT A2_TIPO FROM %table:SA2% WHERE D1_FORNECE = A2_COD AND D1_LOJA = A2_LOJA AND %table:SA2%.D_E_L_E_T_ = '' )
WHEN D1_TIPO = 'D' THEN (SELECT A1_PESSOA FROM %table:SA1% WHERE D1_FORNECE = A1_COD AND D1_LOJA = A1_LOJA AND %table:SA1%.D_E_L_E_T_ = '' ) END AS PESSOA
...
, CASE WHEN D1_TIPO <> 'D' THEN (SELECT A2_COD_MUN FROM %table:SA2% WHERE D1_FORNECE = A2_COD AND D1_LOJA = A2_LOJA AND %table:SA2%.D_E_L_E_T_ = '' )
WHEN D1_TIPO = 'D' THEN (SELECT A1_COD_MUN FROM %table:SA1% WHERE D1_FORNECE = A1_COD AND D1_LOJA = A1_LOJA AND %table:SA1%.D_E_L_E_T_ = '' ) END AS COD_MUN
, CASE WHEN D1_TIPO <> 'D' THEN (SELECT A2_MUN FROM %table:SA2% WHERE D1_FORNECE = A2_COD AND D1_LOJA = A2_LOJA AND %table:SA2%.D_E_L_E_T_ = '' )
WHEN D1_TIPO = 'D' THEN (SELECT A1_MUN FROM %table:SA1% WHERE D1_FORNECE = A1_COD AND D1_LOJA = A1_LOJA AND %table:SA1%.D_E_L_E_T_ = '' ) END AS MUNICIPIO
, CASE WHEN D1_TIPO <> 'D' THEN ''
WHEN D1_TIPO = 'D' THEN (SELECT A1_ZZCANAL FROM %table:SA1% WHERE D1_FORNECE = A1_COD AND D1_LOJA = A1_LOJA AND %table:SA1%.D_E_L_E_T_ = '' ) END AS COD_CANAL
, CASE WHEN D1_TIPO <> 'D' THEN ''
WHEN D1_TIPO = 'D' THEN (SELECT A1_ZZDSCAN FROM %table:SA1% WHERE D1_FORNECE = A1_COD AND D1_LOJA = A1_LOJA AND %table:SA1%.D_E_L_E_T_ = '' ) END AS CANAL
FROM %table:SD1%
...
INNER JOIN %table:NNR% ON D1_FILIAL = NNR_FILIAL AND D1_LOCAL = NNR_CODIGO AND %table:NNR%.D_E_L_E_T_ = ''
//INNER JOIN %table:NNR% ON D1_LOCAL = NNR_CODIGO AND %table:NNR%.D_E_L_E_T_ = ''
...
oFWMsExcel:AddColumn("Entradas",cNomeTabela,"COD_CANAL"   	,1,1) //33
oFWMsExcel:AddColumn("Entradas",cNomeTabela,"CANAL"   	  	,1,1) //34

While !(cAlias)->(Eof())

	//Criando as Linhas
    oFWMsExcel:AddRow("Entradas",cNomeTabela,{TIPO,EMPRESA,FILIAL,TIPONF1,NFISCAL,SERIE,ITEM,PRODUTO,DESCRICAOP,QUANTIDADE,MEDIDA,VLUNIT,VLTOTAL,ARMAZEM,DESCRICAOA,LOTE,DTOC(STOD(VALIDADE)),DTOC(STOD(ENTRADA)),DTOC(STOD(EMISSAO)),TES,FINALIDADE,ESTOQUE,FINANCEIRO,CFOP,CLIFOR,NOME,ESTADO,PESSOA,PEDIDO,ITEMPD,COD_MUN,MUNICIPIO,COD_CANAL,CANAL})
	//oFWMsExcel:AddRow("Entradas",cNomeTabela,{TIPO,EMPRESA,FILIAL,TIPONF1,NFISCAL,SERIE,ITEM,PRODUTO,DESCRICAOP,QUANTIDADE,MEDIDA,VLUNIT,VLTOTAL,ARMAZEM,DESCRICAOA,LOTE,DTOC(STOD(VALIDADE)),DTOC(STOD(ENTRADA)),DTOC(STOD(EMISSAO)),TES,FINALIDADE,ESTOQUE,FINANCEIRO,CFOP,CLIFOR,NOME,ESTADO,PESSOA,PEDIDO,ITEMPD,COD_MUN,MUNICIPIO})
...
WHEN D2_TIPO = 'D' THEN (SELECT A2_NOME FROM %table:SA2% WHERE D2_CLIENTE = A2_COD AND D2_LOJA = A2_LOJA AND %table:SA2%.D_E_L_E_T_ = '' )
WHEN D2_TIPO <> 'D' THEN (SELECT A1_NOME FROM %table:SA1% WHERE D2_CLIENTE = A1_COD AND D2_LOJA = A1_LOJA AND %table:SA1%.D_E_L_E_T_ = '' ) END AS NOME
, CASE WHEN D2_TIPO = 'D' THEN (SELECT A2_EST FROM %table:SA2% WHERE D2_CLIENTE = A2_COD AND D2_LOJA = A2_LOJA AND %table:SA2%.D_E_L_E_T_ = '' )
WHEN D2_TIPO <> 'D' THEN (SELECT A1_EST FROM %table:SA1% WHERE D2_CLIENTE = A1_COD AND D2_LOJA = A1_LOJA AND %table:SA1%.D_E_L_E_T_ = '' ) END AS ESTADO
, CASE WHEN D2_TIPO = 'D' THEN (SELECT A2_TIPO FROM %table:SA2% WHERE D2_CLIENTE = A2_COD AND D2_LOJA = A2_LOJA AND %table:SA2%.D_E_L_E_T_ = '' )
WHEN D2_TIPO <> 'D' THEN (SELECT A1_PESSOA FROM %table:SA1% WHERE D2_CLIENTE = A1_COD AND D2_LOJA = A1_LOJA AND %table:SA1%.D_E_L_E_T_ = '' ) END AS PESSOA
...
//Rafael França - 07/07/20 - Inclusão das informações: Codigo de municipio, Municipio, Canal e Descrição (Cliente)
, CASE WHEN D2_TIPO = 'D' THEN (SELECT A2_COD_MUN FROM %table:SA2% WHERE D2_CLIENTE = A2_COD AND D2_LOJA = A2_LOJA AND %table:SA2%.D_E_L_E_T_ = '' )
WHEN D2_TIPO <> 'D' THEN (SELECT A1_COD_MUN FROM %table:SA1% WHERE D2_CLIENTE = A1_COD AND D2_LOJA = A1_LOJA AND %table:SA1%.D_E_L_E_T_ = '' ) END AS COD_MUN
, CASE WHEN D2_TIPO = 'D' THEN (SELECT A2_MUN FROM %table:SA2% WHERE D2_CLIENTE = A2_COD AND D2_LOJA = A2_LOJA AND %table:SA2%.D_E_L_E_T_ = '' )
WHEN D2_TIPO <> 'D' THEN (SELECT A1_MUN FROM %table:SA1% WHERE D2_CLIENTE = A1_COD AND D2_LOJA = A1_LOJA AND %table:SA1%.D_E_L_E_T_ = '' ) END AS MUNICIPIO
, CASE WHEN D2_TIPO = 'D' THEN ''
WHEN D2_TIPO <> 'D' THEN (SELECT A1_ZZCANAL FROM %table:SA1% WHERE D2_CLIENTE = A1_COD AND D2_LOJA = A1_LOJA AND %table:SA1%.D_E_L_E_T_ = '' ) END AS COD_CANAL
, CASE WHEN D2_TIPO = 'D' THEN ''
WHEN D2_TIPO <> 'D' THEN (SELECT A1_ZZDSCAN FROM %table:SA1% WHERE D2_CLIENTE = A1_COD AND D2_LOJA = A1_LOJA AND %table:SA1%.D_E_L_E_T_ = '' ) END AS CANAL
FROM %table:SD2%
...
INNER JOIN %table:NNR% ON D2_FILIAL = NNR_FILIAL AND D2_LOCAL = NNR_CODIGO AND %table:NNR%.D_E_L_E_T_ = ''
//INNER JOIN %table:NNR% ON D2_LOCAL = NNR_CODIGO AND %table:NNR%.D_E_L_E_T_ = ''
...
oFWMsExcel:AddColumn("Saidas",cNomeTabela,"COD_CANAL"   ,1,1) //33
oFWMsExcel:AddColumn("Saidas",cNomeTabela,"CANAL"   	,1,1) //34
