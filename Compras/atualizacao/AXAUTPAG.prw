...
nOpca := AxInclui("SZS",SZS->(Recno()), 3,, "U_ZSBefore",, "U_ZSTudoOk()", .F., "U_ZSTransactionZS", aButtons, aParam,,,.T.,,,,,)
Return nOpca
...
IF(SUBSTR(EMBARALHA(SZS->ZS_USERLGI,1),3,6) == __cUserID) .AND. SZS->ZS_LIBERAD != "L" .OR. AllTrim(cUserName) $ "Administrador"
	dbSelectArea("SZS")
	nOpca := AxAltera("SZS",SZS->(Recno()), 4,,,,,"U_ZSTudoOk()","U_ZSTransactionZS","U_ZSBefore",aButtons,aParam,,.T.,,,,,)
	Return nOpca 
//	ELSE    
//AxAltera("SZS",SZS->(Recno()), 4,,,,,"U_ZSTudoOk()","U_ZSTransactionZS","U_ZSBefore",aButtons,aParam,,.T.,,,,,)	
...
While !EOF()	
	...
	PEND->ZS_AUTO01,; 	// 18 - Nome do 1บ Aprovador
	PEND->ZS_DEP01,; // 19 - Departamento do 1บ Aprovador
	...
	PEND->ZS_DEP06})// 31 - Departamento do 6บ Aprovador}) // 27 - Codigo do Centro de Custo
...
//oPrint:Say (2200,1130,"FษRIAS"						,oFont12)//TEMPORARIO - FERIAS ELENN
...
If AllTrim(Substring(cUsuario,7,15)) $ "Administrador%Eleni Caldeira (Elenn)%Claudinei Girotti%Josiel Ferreira%Wagner Lima da Silva"
	...
	cQuery += "UNION "
	cQuery += "SELECT C3_NATUREZ AS NATUREZA,SUM(C7_TOTAL + C7_VALFRE + C7_DESPESA - C7_VLDESC) AS VALOR FROM SC7010 "    // Busca Autoriza็ใo de Contratos
	...
	@ 012,001 SAY UPPER("Saldo ap๓s aprova็ใo :  " + cValToChar(nSaldoa)) 
	@ 013,001 SAY "OBSERVAวรO :"
	@ 013,007 GET cObs 
	@ 018,022 BUTTON "Pendente"	SIZE 35,10 ACTION PendenteSZS(dDataLib,cObs)	
	@ 018,032 BUTTON "Liberar" 	SIZE 35,10 ACTION LiberaSZS(dDataLib,cObs)
...
Static Function LiberaSZS(dDataLib,cObs)		//Grava a Nova data no Campo E3_DATA
