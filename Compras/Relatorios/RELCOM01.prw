...
±±ºPrograma  ³ RELCOM01   º Autor ³ BRUNO ALVES         º Data ³  26/03/20º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDescricao ³ CONSULTA DE PEDIDOS/AUTORIZAÇOES POR CONTA SIG NA TELA 	  º±±
±±º          ³ DE APROVAÇÃO DOS DOCUMENTOS 							      º±±
...
User Function RELCOM01(nOpc) //nOpcao 1 = Relatorio; 2 = Consulta na rotina liberação do documento
...
Static Function RunReport(Cabec1,Cabec2,Titulo,nLin)
	...
	Local nTotal	:= 0
	Local lOk		:= .F.
	Local cCampo	:= IIF(nOpcao == 1,"ZY_MES" + MV_PAR03,"ZY_MES" + SUBSTR(DTOS(dDatabase),5,2) )
	Local cMemo		:= ""
	Local nAprov    := 0
	Local nBApro	:= 0
	Local cAprov	:= ""
	...
	cQuery += " AND SCR010.D_E_L_E_T_ = '' AND (CR_DATALIB = '' OR SUBSTRING(CR_DATALIB,1,6) >= '" + IIF(nOpcao == 1,MV_PAR04,SUBSTR(DTOS(dDatabase),1,4) ) + IIF(nOpcao == 1,MV_PAR03,SUBSTR(DTOS(dDatabase),5,2) ) + "' ) "
	cQuery += " UNION "
	...
	cQuery += " AND SUBSTRING(ZS_EMISSAO,1,6) = '" + IIF(nOpcao == 1,MV_PAR04,SUBSTR(DTOS(dDatabase),1,4) ) + IIF(nOpcao == 1,MV_PAR03,SUBSTR(DTOS(dDatabase),5,2) ) + "' "
	...
	While !EOF()
		...
		IF !(TMP1->TIPO == "PC" .AND. (!EMPTY(TMP1->LIBERACAO) .AND. SUBSTR(TMP1->LIBERACAO,1,6) < (IIF(nOpcao == 1,MV_PAR04,SUBSTR(DTOS(dDatabase),1,4) ) + IIF(nOpcao == 1,MV_PAR03,SUBSTR(DTOS(dDatabase),5,2) ))))
			//ELSE
	...
	EndIf
	cQuery += " SUBSTRING(D1_DTDIGIT,1,6) = '" + IIF(nOpcao == 1,MV_PAR04,SUBSTR(DTOS(dDatabase),1,4) ) + IIF(nOpcao == 1,MV_PAR03,SUBSTR(DTOS(dDatabase),5,2) ) + "' "
	...
	For _I := 1 To Len(aPedidos)
		...
		IF STOD(aPedidos[_I,4]) <= IIF(nOpcao == 1,MV_PAR10,MonthSum(dDatabase,3)) .OR. IIF(nOpcao == 1,MV_PAR10,"")
			...
			IF !(aPedidos[_I,3] == "PC" .AND. ALLTRIM(aPedidos[_I,2]) <> IIF(nOpcao == 1,MV_PAR11,"") .AND. (ALLTRIM(aPedidos[_I,1]) == "10007" .OR. ALLTRIM(aPedidos[_I,1]) == "08005")) //Rafael - Filtra apenas os pedido corrente na conta SIG 10008 e 8005 solicitado pela Srta Janaina e aprovado pela Sra Elenn // Wesley - Alterado conta 10008 para 10007 para o novo plano conta sig de 2020
				...
				IF ALLTRIM(aPedidos[_I,2]) $ "017889%017888%017887%017880%017879%017885%017884%017886%017898%017899" .AND. ALLTRIM(aPedidos[_I,2]) <> IIF(nOpcao == 1,MV_PAR11,"")
				...
				If cSIG	!= aPedidos[_I,1]  .AND. !EMPTY(aPedidos[_I,1])
					...
					//IF cSIG < "19999"
					@nLin,011 PSAY Posicione("SZY",1,xFilial("SZY")+PADR(Alltrim(aPedidos[_I,1]),6)+IIF(nOpcao == 1,MV_PAR04,SUBSTR(DTOS(dDatabase),1,4) ),"ZY_DESCRI")
					//ELSE
				...
				ELSE
					//IF cSig < "19999"
					@nLin,017 PSAY (IIF(nOpcao == 1,MV_PAR03,SUBSTR(DTOS(dDatabase),5,2) )) + "/" + (IIF(nOpcao == 1,MV_PAR04,SUBSTR(DTOS(dDatabase),1,4) ))
					//ELSE
					//	IF MV_PAR03 < "07"
			...
			For _I := 1 To Len(aPedidosF)
				...
				IF STOD(aPedidosF[_I,4]) <= MV_PAR10 .OR. EMPTY(MV_PAR10)
					...
					IF !(aPedidosF[_I,3] == "PC" .AND. ALLTRIM(aPedidosF[_I,2]) <> MV_PAR11 .AND. (ALLTRIM(aPedidosF[_I,1]) == "10008" .OR. ALLTRIM(aPedidosF[_I,1]) == "08005")) //Rafael - Filtra apenas os pedido corrente na conta SIG 10008 e 8005 solicitado pela Srta Janaina e aprovado pela Sra Elenn
						...
						nOrcSigF := Posicione("SZY",1,xFilial("SZY")+PADR(Alltrim(cSIGF),6)+IIF(nOpcao == 1,MV_PAR04,SUBSTR(DTOS(dDatabase),1,4) ),@cCampo)
						...
						If cSIGF	!= aPedidosF[_I,1] .AND. lOk
							...
							nLin 	+= 1
							@nLin,001 PSAY Posicione("SZY",1,xFilial("SZY")+PADR(Alltrim(cSIGF),6)+IIF(nOpcao == 1,MV_PAR04,SUBSTR(DTOS(dDatabase),1,4) ),"ZY_DESCRI")
							@nLin,050 PSAY "VL. ORÇADO:"
						...
						If cSIGF	!= aPedidosF[_I,1]  .AND. !EMPTY(aPedidosF[_I,1])
							...
							@nLin,011 PSAY Posicione("SZY",1,xFilial("SZY")+PADR(Alltrim(aPedidosF[_I,1]),6)+IIF(nOpcao == 1,MV_PAR04,SUBSTR(DTOS(dDatabase),1,4) ),"ZY_DESCRI")
							nLin 	+= 1
							@nLin,001 PSAY REPLICATE("-",LIMITE)
							nLin 	+= 1
