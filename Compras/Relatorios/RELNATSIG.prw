...
If AllTrim(Substring(cUsuario,7,15)) $ "Luciano Ribeiro%Eleni Caldeira (Elenn)%Adauto Pereira%bruno alves%Claudinei Girotti%Josiel Ferreira%Wagner Lima%"
...
cQuery += " ,SUM(C7_TOTAL + C7_DESPESA - C7_VLDESC + C7_VALFRE) AS VALOR " //AGRUPADO POR PEDIDO
cQuery += " FROM SC7010 "
...
cQuery += " ,(C7_TOTAL + C7_DESPESA - C7_VLDESC + C7_VALFRE) AS VALOR "
...
For _I := 1 To Len(aPedidos)
	...
	IF STOD(aPedidos[_I,4]) <= MV_PAR10 .OR. EMPTY(MV_PAR10)
		...
		IF !(aPedidos[_I,3] == "PC" .AND. ALLTRIM(aPedidos[_I,2]) <> MV_PAR11 .AND. (ALLTRIM(aPedidos[_I,1]) == "10008" .OR. ALLTRIM(aPedidos[_I,1]) == "08005")) //Rafael - Filtra apenas os pedido corrente na conta SIG 10008 e 8005 solicitado pela Srta Janaina e aprovado pela Sra Elenn
			...
			*/
			...
			nOrcSig := Posicione("SZY",1,xFilial("SZY")+cSIG,@cCampo)
			...
			If cSIG	!= aPedidos[_I,1] .AND. lOk
				...
				//		@nLin,001 PSAY "TOTAL"
				//IF cSIG < "19999"
				@nLin,001 PSAY Posicione("SZY",1,xFilial("SZY")+cSIG,"ZY_DESCRI")
				//ELSE
			...
			If cSIG	!= aPedidos[_I,1]  .AND. !EMPTY(aPedidos[_I,1])
				...
				//IF cSIG < "19999"
				@nLin,011 PSAY Posicione("SZY",1,xFilial("SZY")+aPedidos[_I,1],"ZY_DESCRI")
				//ELSE
			...
			IF aPedidos[_I,3] <> "AM"
				...
				@nLin,191 PSAY STOD(aPedidos[_I,12]) 		//DT LIBERAÇÃO
			...
			ELSE
				...
				@nLin,038 PSAY "                         "  //NOME
				@nLin,180 PSAY cAprov				    	//LIBERADO
				@nLin,192 PSAY STOD(aPedidos[_I,12]) 		//DT LIBERAÇÃO
	...
	For _I := 1 To Len(aPedidosF)
		...
		IF STOD(aPedidosF[_I,4]) <= MV_PAR10 .OR. EMPTY(MV_PAR10)
			...
			IF !(aPedidosF[_I,3] == "PC" .AND. ALLTRIM(aPedidosF[_I,2]) <> MV_PAR11 .AND. (ALLTRIM(aPedidosF[_I,1]) == "10008" .OR. ALLTRIM(aPedidosF[_I,1]) == "08005")) //Rafael - Filtra apenas os pedido corrente na conta SIG 10008 e 8005 solicitado pela Srta Janaina e aprovado pela Sra Elenn
				...
				nOrcSigF := Posicione("SZY",1,xFilial("SZY")+cSIGF,@cCampo)
				...
				If cSIGF	!= aPedidosF[_I,1] .AND. lOk
					...
					nLin 	+= 1
					@nLin,001 PSAY Posicione("SZY",1,xFilial("SZY")+cSIGF,"ZY_DESCRI")
					@nLin,050 PSAY "VL. ORÇADO:"
				...
				If cSIGF	!= aPedidosF[_I,1]  .AND. !EMPTY(aPedidosF[_I,1])
					...
					@nLin,011 PSAY Posicione("SZY",1,xFilial("SZY")+aPedidosF[_I,1],"ZY_DESCRI")
					nLin 	+= 1
					@nLin,001 PSAY REPLICATE("-",LIMITE)
					nLin 	+= 1
				...
				IF aPedidosF[_I,3] <> "AM"
					...
					@nLin,179 PSAY cAprovF						//LIBERADO
					@nLin,191 PSAY STOD(aPedidosF[_I,12]) 		//DT LIBERAÇÃO
					@nLin,206 PSAY aPedidosF[_I,13] PICTURE "@E 999,999,999.99"	//VALOR
				ELSE
					@nLin,017 PSAY (MV_PAR03) + "/" + (MV_PAR04)
					@nLin,026 PSAY aPedidosF[_I,5]
					@nLin,038 PSAY "                         "  //NOME
					@nLin,180 PSAY cAprovF				    	//LIBERADO
					@nLin,192 PSAY STOD(aPedidosF[_I,12]) 		//DT LIBERAÇÃO
