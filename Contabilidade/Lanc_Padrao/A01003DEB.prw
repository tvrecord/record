...
±±ºPrograma  ³A01003DEBºAutor  ³Microsiga           º Data ³  01/03/13   º±±
...
User Function A01003DEB()


Private cConta := ""
Private cGrupo := ""
Private cSRVConta := IIF(SRZ->RZ_PD=="760".AND.SRZ->RZ_VAL<0,POSICIONE("SRV",1,XFILIAL("SRV")+SRZ->RZ_PD,"RV_CONTA"),"" )

_aArea := GetArea()    	

If cSRVConta >= '41' .AND. cSRVConta <= '439999999'
	
	cGrupo := Posicione("CTT",1,xFilial("CTT") + IIF(SUBSTR(POSICIONE("SRV",1,xFilial("SRV")+SRZ->RZ_PD,"RV_CONTA"),1,1)=="4",SRZ->RZ_CC,""), "CTT_GRUPO")// Busca o grupo do plano de contas no cadastro do centro de custo
