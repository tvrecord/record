...
±±ºPrograma  ³A01004DEBºAutor  ³Microsiga           º Data ³  01/03/13   º±±
...
User Function A01004DEB()


Private cConta := ""
Private cGrupo := ""
Private cSRVConta := iif(SRZ->RZ_PD<>"760",POSICIONE("SRV",1,"  "+SRZ->RZ_PD,"RV_CTACRED"),"")

_aArea := GetArea()    	

If cSRVConta >= '41' .AND. cSRVConta <= '439999999'
	
	cGrupo := Posicione("CTT",1,xFilial("CTT") + IF(SUBSTR(POSICIONE("SRV",1,xFilial("SRV")+SRZ->RZ_PD,"RV_CTACRED"),1,1)=="4",SRZ->RZ_CC,""), "CTT_GRUPO")// Busca o grupo do plano de contas no cadastro do centro de custo
