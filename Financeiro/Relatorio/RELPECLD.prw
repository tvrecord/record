...
Static Function RunReport(Cabec1,Cabec2,Titulo,nLin)
	...
	IF MV_PAR01 <> 3		
		IF MV_PAR02 == 1  //06/05/19 - Rafael França - Colocado para atender a solicitação de SP no relatorio Aging List. O objetivo da mudança é separar os titulos do governo dos demais. 
			cFiltro += "AND E1_NATUREZ NOT IN ('1101001','110100101','1101012','110101701','110101702','1101019','1101045','110104701','1101049','1101048','1101006',1101007,'1101008','1101042','1101050') " 	
		ELSEIF MV_PAR02 == 2
			cFiltro += "AND E1_NATUREZ IN ('1101001','110100101','1101012','110101701','110101702','1101019','1101045','110104701','1101049','1101048') " 	      
		ELSEIF MV_PAR02 == 3
	...
	IF MV_PAR01 == 1	
		...
		WHILE nCol <= MV_PAR04 //Verifica a quantidade de colunas necessarias ate o limite 
			...
			cCol 	:= STRZERO(nCol,3) + "_DFM"
			cCol1 	:= "%_" + STRZERO(nCol,3) + "_DFM"
	...
	ELSEIF MV_PAR01 == 3
		...
		aadd( _aCExcel , {"STATUS" 			, "C" , 01 , 00 }) //08
		aadd( _aCExcel , {"LEGENDA"  		, "C" , 12 , 00 }) //09 
		aadd( _aCExcel , {"DFM"  			, "N" , 04 , 00 }) //10
		aadd( _aCExcel , {"VALOR"  			, "N" , 12 , 02 }) //11	
		aadd( _aCExcel , {"VLLIQUIDO"		, "N" , 12 , 02 }) //12	
		aadd( _aCExcel , {"VLBAIXADO"		, "N" , 12 , 02 }) //13	
		aadd( _aCExcel , {"CLIENTE"       	, "C" , 25 , 00 }) //14	
		aadd( _aCExcel , {"NATUREZA"   		, "C" , 20 , 00 }) //15										
		aadd( _aCExcel , {"EMISSAO"			, "D" , 10 , 00 }) //16
		aadd( _aCExcel , {"TITULO"			, "C" , 15 , 00 }) //17	
		aadd( _aCExcel , {"REFERENCIA"		, "D" , 10 , 00 }) //18
		aadd( _aCExcel , {"CLASSIFICACAO"	, "C" , 12 , 00 }) //19				
		aadd( _aCExcel , {"ANOBX"			, "C" , 04 , 00 }) //20	
		aadd( _aCExcel , {"MESBX"			, "C" , 02 , 00 }) //21		
		aadd( _aCExcel , {"DESCRI BX"		, "C" , 12 , 00 }) //22									
		...
		cQuery += ",CASE " 
		cQuery += "WHEN E1_NATUREZ NOT IN ('1101001','110100101','1101012','110101701','110101702','1101019','1101045','110104701','1101049','1101048','1101006',1101007,'1101008','1101042','1101050') THEN 'PRIVADO' "
		cQuery += "WHEN E1_NATUREZ IN ('1101001','110100101','1101012','110101701','110101702','1101019','1101045','110104701','1101049','1101048') THEN 'GOVERNO' "
		...
		While TMP2->(!Eof())
			...
			IF nDAtraso > 0 .AND. nDAtraso <> 9999 // Verficia se o atraso e maior do que o limite de pagamento da planilha - MV_PAR04
				...
				cCol        := "ATE " + STRZERO(nCol * MV_PAR03,3) + " DFM"
			...
			_aItem[07]      := dBaixa			
			_aItem[08]      := IIF(nCol == 9999,"X",STRZERO(nCol,2))
			_aItem[09]      := cCol	
			_aItem[10]      := nDAtraso	
			_aItem[11]      := TMP2->VALOR	
			_aItem[12]      := TMP2->VLLIQUI
			_aItem[13]      := nVlBx	
			_aItem[14]      := TMP2->CLIENTE
			_aItem[15]      := TMP2->NATUREZA				
			_aItem[16]      := STOD(TMP2->EMISSAO)
			_aItem[17]      := TMP2->TITULO	
			_aItem[18]      := MV_PAR06
			_aItem[19]      := TMP2->CLASSIF
			_aItem[20]      := year(dBaixa)
			_aItem[21]      := month(dBaixa)	
			_aItem[22]		:= UPPER(MesExtenso(month(dBaixa)))
...
Static Function ValidPerg(cPerg)
	...
	AADD(aRegs,{cPerg,"05","Ano Referencia:	","","","mv_ch05","C",19,0,0,"G","","mv_par05","","","","","","","","","","","","","","","","","","","","","","","","",""})	
