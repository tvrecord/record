...
cQuery += "SED010.D_E_L_E_T_ <> '*' AND "
cQuery += "ED_NATGER BETWEEN '" + (MV_PAR24) + "' AND '" + (MV_PAR25) + "' AND "
cQuery += "'"+DTOS(MV_PAR08)+"' >= C3_DATPRI AND '"+ DTOS(MONTHSUB(MV_PAR08,1)) +"' <= C3_DATPRF AND " //Rafael FranÁa - 17/08/2020 - Valida vigencia do contrato, 07/06/2022 - Alterado para pegar contratos finalizados no mÍs anterior e o vencimento nesse mÍs.
cQuery += "C3_ENCER <> 'E' AND "
cQuery += "C3_RENOVA = '' AND "
cQuery += "C3_PERMUTA <> '1' AND "
cQuery += "C3_FLUXO = '1' AND "
...
While !EOF() .AND. DATAVALIDA(STOD(REALIZ->E5_DATA)) < MV_PAR27
	...
	If (TemBxCanc(REALIZ->(E5_PREFIXO+E5_NUMERO+E5_PARCELA+E5_TIPO+E5_CLIFOR+E5_LOJA+E5_SEQ)) .OR. REALIZ->E5_ORIGEM = "FINA100" ) .AND. REALIZ->E5_NATUREZ <> "1205005" .AND. REALIZ->E5_NATUREZ <> "1205020"
		REALIZ->(dbSkip())
		Loop
	EndIf
...
While !EOF()
	...
	IF ALLTRIM(PREV->ZA6_NATURE) == "1205004" .OR. ALLTRIM(PREV->ZA6_NATURE) == "1205005" .AND. ALLTRIM(PREV->ZA6_NATURE) == "1205020"
