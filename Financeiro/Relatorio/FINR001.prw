...
Static Function ImpProxPag()
	...
	oPrint:Say( nLin,088, "NF",oFonteN)
	oPrint:Say( nLin,116, "Emissao",oFonteN)
	oPrint:Say( nLin,160, "Venc.",oFonteN)
	oPrint:Say( nLin,200, "Cliente",oFonteN)
	oPrint:Say( nLin,300, "Agência",oFonteN)
	oPrint:Say( nLin,410, "Valor",oFonteN)
...
Static Function GetData(cPPeriodo,cPPracaDe,cPPracaAte,cPRpDe,cPRpAte)
	...
	BeginSql Alias cTmp1
		...
		WHERE
			...
			AND ZAG.D_E_L_E_T_ = ''
			AND ZAH.D_E_L_E_T_ = ''
//Rafael França - 24/03/22 - Remove as duplicidades do relatório
